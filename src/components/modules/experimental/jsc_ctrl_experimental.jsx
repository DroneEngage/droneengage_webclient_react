...
export class ClssCtrlExperimental extends React.Component {
    constructor(props) {
        ...
        this.jsonData = loadConfigurationFiles('mavlink.json');
    }
    ...
    fn_applyMavlinkConfiguration (p_contents)
    {
        ...
        js_globals.v_andruavFacade.API_doModuleConfigAction(v_andruavUnit, js_andruavMessages.CONST_TYPE_CONFIG_ACTION_SHUT_DOWN_HW);
    }
            ...
            <div key={v_andruavUnit.getPartyID() + 'exp_1'} className="col-12 mt-1">
                <div key={v_andruavUnit.getPartyID() + 'exp_2'} className='row al_l css_margin_zero d-flex '>
                    <div key={v_andruavUnit.getPartyID() + 'exp_21'} className='col-4 col-sm-3 user-select-none '>
                        <p key={v_andruavUnit.getPartyID() + 'exp_211'} className=' rounded-3 txt-theme-aware bg-danger cursor_hand textunit_nowidth al_c' title='Local Comm Server' onClick={() => this.fn_ConnectToLocalCommServer(v_andruavUnit)}>LWSC</p>
                    </div>
                    <div key={v_andruavUnit.getPartyID() + 'exp_22'} className='col-4 col-sm-3 user-select-none '>
                        <p key={v_andruavUnit.getPartyID() + 'exp_212'} className=' rounded-3 txt-theme-aware bg-primary cursor_hand textunit_nowidth al_c bi bi-upc-scan' title='Scan Nearby WIFI' onClick={() => this.fn_scanP2P(v_andruavUnit)}> WIFI Scan</p>
                    </div>
            ...
            <div key={v_andruavUnit.getPartyID() + "_ctl_exp"} className={this.props.className}>
                ...
                <div key={v_andruavUnit.getPartyID() + 'exp_1'} className='row css_margin_zero padding_zero '>
                    <div key={v_andruavUnit.getPartyID() + 'sdr_112'} className='row css_margin_zero padding_zero '>
                        ...
                        <p key={v_andruavUnit.getPartyID() + 'exp_211'} className=' col-2 rounded-3 txt-theme-aware bg-danger cursor_hand textunit_nowidth al_c' title='Reset P2P HW' onClick={() => this.fn_ConnectToLocalCommServer(v_andruavUnit)}>LWSC</p>
